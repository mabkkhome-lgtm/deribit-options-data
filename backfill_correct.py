#!/usr/bin/env python3
"""
Backfill last 48 hours with CORRECT Thales logic
"""
import requests
...
def fetch_for_time(target_time):
    """Fetch data for a specific time using exact fetch_historical.py logic"""
    start = target_time.replace(hour=0, minute=0, second=0, microsecond=0)
    end = target_time.replace(minute=59, second=59, microsecond=0)
    ...
    try:
        ...
        current_code = (target_time - epoch).days
...
print("Backfilling 48 hours with CORRECT Thales logic...")
print("This will take a few minutes...")

now = datetime.now(timezone.utc)
results = []

# Go back 48 hours, every 30 minutes (96 points)
for i in range(96):
    target_time = now - timedelta(minutes=30 * i)
    result = fetch_for_time(target_time)
    
    if result:
        ts = target_time.strftime('%Y-%m-%dT%H:%M')
        results.append(f"{ts},{result['r']},{result['s']},{result['bg']},{result['sg']}")
        print(f"{ts}: R={result['r']}, S={result['s']}")
    
    if i % 10 == 0:
        print(f"Progress: {i}/96 points...")
...
print(f"\n✅ Backfilled {len(results)} points with CORRECT calculation logic")
